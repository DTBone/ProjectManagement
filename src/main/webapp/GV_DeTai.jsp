                        ...
                        <c:forEach var="dt" items="${dtaiofgv}">
                            ...
                            <tr>
                                ...
                                <td class="column4"><fmt:parseDate var="localDate"
                                                                   value="${dt.getNgayKetThuc()}" pattern="yyyy-MM-dd"/>
                                    <fmt:formatDate
                                            value="${localDate}" pattern="dd/MM/yyyy"
                                            var="formattedDate"/> <c:out value="${formattedDate}"/></td>
                                ...
                                <td class="column9"><fmt:parseDate var="localDate"
                                                                   value="${dt.getNgayNop()}" pattern="yyyy-MM-dd"/>
                                    <fmt:formatDate
                                            value="${localDate}" pattern="dd/MM/yyyy"
                                            var="formattedDate"/> <c:out value="${formattedDate}"/></td>
                                <td class="column10">
                                    <c:if test="${empty dt.getLinknop()}">
                                    <c:if test="${not empty dt.getNgayKetThuc()}">
                                        <a href="#" onclick="showGiaHan('<c:out value='${dt.getMaDT()}' />')">Gia hạn</a>
                                    <c:if test="${nop}">
...
<%--<script>--%>
<%--    function showGiaHan(maDT) {--%>
<%--        if (!isValidInput(maDT)) {--%>
<%--            alert("Invalid input!");--%>
<%--            return;--%>
<%--        }--%>
<%--        var url = "ShowGiaHan?id=" + safeEncodeURIComponent(maDT);--%>
