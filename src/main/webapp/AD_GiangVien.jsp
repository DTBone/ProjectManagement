                            ...
                            <c:forEach var="teacher" items="${listGiangVien}">
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <a href="#" onclick="showGiangVien('<c:out value='${teacher.maTK}'/>')">Xem <i class="fas fa-eye"> - </i></a>
                                        <a href="#" onclick="updateGiangVien('<c:out value='${teacher.maTK}'/>')">Sửa <i class="fas fa-edit"> - </i></a>
                                        <a href="#" onclick="deleteGiangVien('<c:out value='${teacher.maTK}'/>')">Xóa <i class="fas fa-trash"> - </i></a>
<%--                                        <a--%>
<%--                                                href="<%=request.getContextPath()%>/AD/show_giangvien?mtk=<c:out value='${teacher.maTK}' />">--%>
<%--                                            <i class="fas fa-eye"> - </i>--%>
<%--                                            <!-- Biểu tượng mắt hoặc bạn có thể chọn biểu tượng khác -->--%>
<%--                                        </a>--%>

<%--                                        <a--%>
<%--                                                href="<%=request.getContextPath()%>/AD/show_updategv?matkupdate=<c:out value='${teacher.maTK}' />">--%>
<%--                                            <i class="fas fa-edit"> - </i>--%>
<%--                                            <!-- Biểu tượng sửa hoặc bạn có thể chọn biểu tượng khác -->--%>
<%--                                        </a>--%>
                            ...
                            <form action="<%=request.getContextPath()%>/AD/new_giangvien"
                                ...
                                <div class="modal-body">
                                    <div class="form-group">
                                        <label>Mã Giảng Viên</label> <input name="MaGV" type="text" class="form-control" required  maxlength="10">
                                    </div>
                                    <div class="form-group">
...
<%--<script>--%>
<%--    function showGiangVien(maDT) {--%>
<%--        if (!isValidInput(maDT)) {--%>
<%--            alert("Invalid input!");--%>
<%--            return;--%>
<%--        }--%>
<%--        if(maDT === 'show_giangvien'){--%>
<%--            alert("Invalid input!");--%>
<%--            return;--%>
<%--        }--%>
<%--        var url = "<%=request.getContextPath()%>/AD/show_giangvien?mtk=" + safeEncodeURIComponent(maDT);--%>
<%--        window.location.href = url;--%>
<%--    }--%>
<%--    function updateGiangVien(maDT) {--%>
<%--        if (!isValidInput(maDT)) {--%>
<%--            alert("Invalid input!");--%>
<%--            return;--%>
<%--        }--%>
<%--        var url = "<%=request.getContextPath()%>/AD/show_updategv?matkupdate=" + safeEncodeURIComponent(maDT);--%>
<%--        window.location.href = url;--%>
<%--    }--%>
<%--    function deleteGiangVien(maDT) {--%>
<%--        if (!isValidInput(maDT)) {--%>
<%--            alert("Invalid input!");--%>
<%--            return;--%>
<%--        }--%>
<%--        var url = "<%=request.getContextPath()%>/AD/delete_giangvien?mtkvisible=" + safeEncodeURIComponent(maDT);--%>
<%--        window.location.href = url;--%>
<%--    }--%>
...
<%--        var regex = /^[a-zA-Z0-9]+$/; // Chỉ chấp nhận các ký tự từ a-z, A-Z và 0-9--%>
<%--        if (!regex.test(input)) {--%>
<%--            return false;--%>
<%--        }--%>
